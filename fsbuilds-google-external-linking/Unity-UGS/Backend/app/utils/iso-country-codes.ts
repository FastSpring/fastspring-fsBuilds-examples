// ISO 3166-1 alpha-2 -> alpha-3 country code mapping. FastSpring's webhook
// gives a two-letter country code (address.country); Apple's External
// Purchase Server API requires a three-letter one (taxCountry). Static
// reference data, not an external API — safe to hardcode and doesn't need
// re-verification the way a live endpoint would.
